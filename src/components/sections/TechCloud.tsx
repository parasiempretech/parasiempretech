...
export default function TechCloud({
  ...
  // OPTIMIZACIÓN: Usamos useMemo para asegurarnos de que la lista de iconos no se recalcule
  const techStack = useMemo(() => EXCELLENCE_STACK, []);
        ...
        {techStack.map((tech) => {
          ...
          return (
            <motion.div
              ...
              // Añadimos 'relative' para que el tooltip se posicione correctamente
              className="flex flex-col items-center justify-center text-center group cursor-pointer relative"
              whileHover={{ scale: 1.15, rotate: 2 }}
            >
              {/* Tooltip flotante (NUEVA MEJORA DE UX) */}
              <motion.div
                className="absolute bottom-full mb-3 px-3 py-1 bg-white rounded-full shadow-lg whitespace-nowrap text-black text-xs font-semibold opacity-0 group-hover:opacity-100 transition-opacity duration-300"
                initial={{ y: 10 }}
                animate={{ y: 0 }}
              ...
              <motion.div
                // Animación de pulsación sutil y continua
                animate={{ y: [0, -6, 0] }}
                transition={{
