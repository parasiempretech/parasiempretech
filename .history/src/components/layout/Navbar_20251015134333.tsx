...
export default function Navbar() {
  ...
  const isFAQActive = pathname === "/contacto" || pathname === "/faq"; // ✅ Detecta si estás en Preguntas frecuentes
    ...
    <header className="sticky top-0 z-50 w-full border-b border-white/10 bg-[#0A0A0A]/80 backdrop-blur-lg">
      <div className="container mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        <nav className="hidden md:flex items-center space-x-2">
          ...
          {/* --- Preguntas frecuentes con subrayado activo --- */}
          <Link
            href="/contacto"
            aria-current={isFAQActive ? "page" : undefined}
            className={`ml-3 inline-flex items-center justify-center rounded-lg px-4 py-2 text-sm font-semibold transition-all duration-200 ${
              isFAQActive
                ? "text-white border-b-2 border-emerald-400"
                : "text-white bg-gradient-to-r from-emerald-500/20 to-sky-500/20 hover:scale-105"
            }`}
          >
      ...
      {isMobileMenuOpen && (
        <div className="border-t border-white/10 md:hidden bg-[#0A0A0A]/95 backdrop-blur-xl">
          <div className="flex flex-col space-y-1 px-2 pb-3 pt-2">
            ...
            {/* --- Preguntas frecuentes en móvil --- */}
            <Link
              href="/contacto"
              onClick={() => setIsMobileMenuOpen(false)}
              className={`mt-2 block rounded-md px-3 py-3 text-center text-base font-medium transition-all duration-200 ${
                isFAQActive
                  ? "text-white border-b-2 border-emerald-400"
                  : "text-white bg-gradient-to-r from-emerald-500/20 to-sky-500/20"
              }`}
            >
              Preguntas frecuentes
            </Link>
