...
type TechGroup = {
  ...
  example: string;
  techs: string;
  // Tailwind utilities (precompuestas; no generamos clases dinámicas)
  gradient: string; // ej: "from-cyan-400/30 via-transparent to-transparent"
  shadowColor: string; // ej: "shadow-cyan-500/30"
};

/* =========================
   Animaciones (sin ease para evitar errores TS)
   ========================= */
const cardAppear = {
...
const TECH_GROUPS: TechGroup[] = [
  ...
  },
  {
    ...
    keyBenefit: "ESCALA",
    example:
      "Ej. Venta masiva de entradas sin colas ni errores durante el lanzamiento.",
...
export default function Tecnologias() {
  ...
  const renderCard = (group: TechGroup, i: number, isDouble: boolean) => (
    ...
    >
      {/* Glow y gradiente interno (seguro, sin estilos exóticos) */}
      <motion.div
        className={`pointer-events-none absolute inset-0 bg-gradient-to-br ${group.gradient} opacity-20`}
        style={{ x: mouseX, y: mouseY, scale: 1.05 }}
      />
      <motion.div
        className="pointer-events-none absolute -top-24 -left-24 h-64 w-64 rounded-full bg-white/5 blur-3xl"
        initial={{ opacity: 0 }}
      ...
      <div className="relative h-full flex flex-col justify-between">
        ...
        <h4
          className={[
            "font-extrabold text-white/5 text-center mt-auto mb-4",
            isDouble ? "text-7xl" : "text-5xl lg:text-6xl",
