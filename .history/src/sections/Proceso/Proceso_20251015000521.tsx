...
/* =========================================================
   Animaciones base (sin `ease` para evitar errores de tipos)
   ========================================================= */
const appear = {
...
function PillCard({
  ...
  return (
    <div
      className={`rounded-2xl border ${tone.border} bg-gradient-to-br ${tone.from} ${tone.to} p-6 text-center shadow-[0_12px_36px_rgba(0,0,0,0.25)]`}
      role="group"
      aria-label={title}
    >
...
export default function Proceso() {
  return (
    ...
    >
      ...
      {/* Encabezado */}
      <motion.div {...appear} transition={{ duration: 0.5 }}>
      ...
      <div className="mt-16 space-y-18 relative z-10">
        {STEPS.map((s, i) => (
          <motion.article
            key={s.id}
            {...appear}
            transition={{ duration: 0.5, delay: i * 0.08 }}
            className="grid grid-cols-1 lg:grid-cols-[2fr,1fr] gap-8 items-stretch rounded-3xl border border-white/10 bg-white/[0.06] p-6 md:p-8 shadow-[0_10px_30px_rgba(0,0,0,0.18)] backdrop-blur-sm"
          >
            {/* IZQUIERDA */}
            <div>
                ...
                <div className="rounded-2xl border border-white/10 bg-white/5 p-5">
                  ...
                  <ul className="space-y-2 text-sm text-gray-300">
                    {s.examples.map((e, idx) => (
                      <li key={idx} className="flex gap-2 leading-relaxed">
                        <span className="text-[#00BFFF] mt-[2px]">●</span>
                        <span>{e}</span>
                      </li>
                    ))}
                  </ul>
            ...
            <div className="flex flex-col justify-between gap-6">
              <PillCard
                title="Fase clave"
                desc="Dirección compartida y foco en valor."
                tone={s.rightTone}
              />
              <PillCard
                title="Resultado esperado"
                desc="Entrega sólida, medible y lista para crecer."
                tone={s.rightTone}
              />
            </div>
          </motion.article>
