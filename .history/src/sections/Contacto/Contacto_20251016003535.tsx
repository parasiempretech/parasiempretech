...
// Aquí usamos los mismos paths SVG, pero el concepto es usar Lucide o similar
...
// ===================================================================
//                        ✨ HOOK DE FORMULARIO CORREGIDO ✨
// ===================================================================

type FormStatus = "idle" | "loading" | "success" | "error" | "validation-error"; // Nuevo estado para error de validación
  ...
  const handleSubmit = useCallback(
    async (e: React.FormEvent) => {
      ...
      if (Object.keys(validationErrors).length > 0) {
        ...
        setStatus("validation-error"); // Usar el nuevo estado específico
        return;
      }
      ...
      try {
        ...
        if (response.ok) {
          ...
          setStatus("error"); // Error de la API/servidor
        }
      } catch (error) {
        console.error("Error al enviar el formulario:", error);
        setStatus("error"); // Error de red/catch
...
// Tipado más limpio para FormInput
...
export default function Contacto() {
  ...
  const { buttonText, buttonClasses, isDisabled } = useMemo(() => {
    let text = "Enviar Mensaje";
    let classes = "";
    // CORRECCIÓN 2: Tipado correcto de 'isDisabled'
    let disabledState = status === "loading" || status === "success";
    ...
    switch (status) {
      ...
      case "success":
        ...
        break;
      case "error":
      case "validation-error": // Manejar ambos errores de forma similar en el botón
        text = "Reintentar Envío";
        classes = "bg-rose-600/80 hover:bg-rose-600";
        disabledState = status === "success"; // Solo deshabilitar en success
        break;
      default:
        classes =
    ...
    return {
      buttonText: text,
      // Se devuelve la cadena de clases sin el estado 'isDisabled'
      ...
      isDisabled: disabledState, // La propiedad isDisabled se devuelve como boolean
    };
  }, [status]);

  // Definición de las redes sociales para mapeo (sin cambios)
  const socialMedia = [
    {
  ...
  // CORRECCIÓN 1: El estado 'validation-error' no es un estado de éxito, por eso la corrección.
  const isErrorStatus = status === "error" || status === "validation-error";
      ...
      >
        ...
        >
          ...
          <div className="pt-2">
            ...
            {/* CORRECCIÓN 1: Usa la nueva variable isErrorStatus */}
            {isErrorStatus && (
          ...
          {/* Botón principal, usando las propiedades dinámicas */}
          <motion.button
            type="submit"
            className={buttonClasses}
            // CORRECCIÓN 2: Se usa la propiedad 'isDisabled' de tipo boolean devuelta por useMemo
            disabled={isDisabled}
            whileHover={
              status === "idle"
